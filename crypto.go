...
type DecryptedError struct {
	...
	// HoldTimes is an array of hold times reported by each node on the
	// error path.
	HoldTimes []uint32
}
...
var (
	// errInitWithAttrData is returned when initial error encryption is
	// requested while attribution data is already present.
	errInitWithAttrData = errors.New("unable to encrypt, cannot " +
		"initialize error with existing attribution data")

	// errInitialDataTooSmall is returned when the initial error data is
	// smaller than the minimum padded onion error length.
	errInitialDataTooSmall = errors.New("initial data size less than " +
		"minimum padded onion error length")
)
...
func (o *OnionErrorEncrypter) EncryptError(initial bool, legacyData []byte,
	...
	if initial && attrData != nil {
		return nil, nil, errInitWithAttrData
	}
	...
	if initial {
		if len(legacyData) < minPaddedOnionErrorLength {
			return nil, nil, fmt.Errorf("%w: min %d",
				errInitialDataTooSmall,
...
func (o *OnionErrorEncrypter) shiftHmacsRight(hmacs []byte) {
	...
	// needed.
	srcIdx := total - 2 //nolint:mnd // Second-to-last hmac slot.
	destIdx := total - 1
